...
const calculateColor = (baseColor, max, value) => {
  if (max === 0) { return baseColor }

  const factor = value / max
  const col = Color(baseColor)

  return factor > 0.5 ? col.darken(factor) : col.lighten(factor)
}

/**
* @returns func - a function that generates new tiles with proper color
*/
const generateTiles = (baseColor, max) => (idx, value) => (
  <div
    key={idx}
    style={{
      backgroundColor: calculateColor(baseColor, max, value),
      borderRadius: '1px',
      height: '10px',
      margin: '1px',
      width: '10px',
  ...
  constructor(props) {
    ...
    this.props.fetchData()
      .then(data => {
        ...
        this.setState({
          data: data,
          tileFactory: generateTiles(this.props.tileColor, max),
        })
      })
  }
